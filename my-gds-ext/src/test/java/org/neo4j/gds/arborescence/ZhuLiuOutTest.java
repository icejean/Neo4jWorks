...
class ZhuLiuOutTest {
    ...
    void testMinimumFromA() {
    	SpanningTree mst = (new ZhuLiuOut(graph, PrimOperators.MIN_OPERATOR, a, ProgressTracker.NULL_TRACKER).compute());
    ...
    void testMinimumFromB() {
    	SpanningTree mst = (new ZhuLiuOut(graph, PrimOperators.MIN_OPERATOR, b, ProgressTracker.NULL_TRACKER).compute());
    	HugeLongArray parent = mst.parentArray();
    ...
    void testMaximumFromA() {
    	SpanningTree mst = (new ZhuLiuOut(graph, PrimOperators.MAX_OPERATOR, a, ProgressTracker.NULL_TRACKER).compute());
    	HugeLongArray parent = mst.parentArray();        
    	assertEquals(5, mst.effectiveNodeCount());
        assertEquals(-1,parent.get(0));
    ...
    void testMaximumFromB() {
    	SpanningTree mst = (new ZhuLiuOut(graph, PrimOperators.MAX_OPERATOR, b, ProgressTracker.NULL_TRACKER).compute());
    	HugeLongArray parent = mst.parentArray();            	
    	assertEquals(0, mst.effectiveNodeCount());
        assertEquals(-1,parent.get(0));
        assertEquals(-1,parent.get(1));
