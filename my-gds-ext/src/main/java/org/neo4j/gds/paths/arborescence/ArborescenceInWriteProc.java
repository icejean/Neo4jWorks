...
public class ArborescenceInWriteProc extends BaseProc {

    static final String procedure = "gds.arborescence.in.write";

    static final String DESCRIPTION = "Custom algorithm";

    @Procedure(value = procedure, mode = WRITE)
    @Description(DESCRIPTION)
    public Stream<ArborescenceInWriteResult> compute(
        @Name(value = "graphName") String graphName,
        @Name(value = "configuration", defaultValue = "{}") Map<String, Object> configuration
    ) {
        return new ProcedureExecutor<>(
            new ArborescenceInWriteSpec(transactionContext()),
            executionContext()
