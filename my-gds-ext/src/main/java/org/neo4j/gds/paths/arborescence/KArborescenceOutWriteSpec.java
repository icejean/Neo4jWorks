...
import org.neo4j.gds.arborescence.KArborescenceOutAlgorithmFactory;
import org.neo4j.gds.arborescence.ZhuLiuOutK;

import java.util.stream.Stream;

import static org.neo4j.gds.executor.ExecutionMode.MUTATE_RELATIONSHIP;

@GdsCallable(name = "gds.arborescence.out.k.write",
             description = KArborescenceInWriteProc.DESCRIPTION,
             executionMode = MUTATE_RELATIONSHIP)
public class KArborescenceOutWriteSpec implements AlgorithmSpec<ZhuLiuOutK, SpanningTree, KArborescenceConfig, Stream<KArborescenceOutWriteResult>, KArborescenceOutAlgorithmFactory<KArborescenceConfig>> {

    private final TransactionContext txContext;

    public KArborescenceOutWriteSpec(TransactionContext txContext) {
        this.txContext = txContext;
    }

    public KArborescenceOutWriteSpec() {
        this.txContext = null;
    }
    ...
    @Override
    public KArborescenceOutAlgorithmFactory<KArborescenceConfig> algorithmFactory(ExecutionContext executionContext) {
        return new KArborescenceOutAlgorithmFactory<>();
    }
    ...
    public ComputationResultConsumer<ZhuLiuOutK, SpanningTree, KArborescenceConfig, Stream<KArborescenceOutWriteResult>> computationResultConsumer() {

        return (computationResult, executionContext) -> {
            KArborescenceOutWriteResult.Builder builder = new KArborescenceOutWriteResult.Builder();
            ...
            ZhuLiuOutK zhuliu = computationResult.algorithm();
