...
import org.neo4j.gds.arborescence.ArborescenceOutAlgorithmFactory;
import org.neo4j.gds.arborescence.ZhuLiuOut;
import java.util.stream.Stream;

import static org.neo4j.gds.executor.ExecutionMode.MUTATE_RELATIONSHIP;

@GdsCallable(name = "gds.arborescence.out.write",
             description = ArborescenceInWriteProc.DESCRIPTION,
             executionMode = MUTATE_RELATIONSHIP)
public class ArborescenceOutWriteSpec implements AlgorithmSpec<ZhuLiuOut, SpanningTree, ArborescenceConfig, Stream<ArborescenceOutWriteResult>, ArborescenceOutAlgorithmFactory<ArborescenceConfig>> {

    private final TransactionContext txContext;

    public ArborescenceOutWriteSpec(TransactionContext txContext) {
        this.txContext = txContext;
    }

    public ArborescenceOutWriteSpec() {
        this.txContext = null;
    }
    ...
    @Override
    public String name() {
        return "ArborescenceOutWrite";
    }

    @Override
    public ArborescenceOutAlgorithmFactory<ArborescenceConfig> algorithmFactory(ExecutionContext executionContext) {
        return new ArborescenceOutAlgorithmFactory<>();
    }

    @Override
    public NewConfigFunction<ArborescenceConfig> newConfigFunction() {
        return (__, config) -> ArborescenceConfig.of(config);

    }

    public ComputationResultConsumer<ZhuLiuOut, SpanningTree, ArborescenceConfig, Stream<ArborescenceOutWriteResult>> computationResultConsumer() {

        return (computationResult, executionContext) -> {
            ArborescenceOutWriteResult.Builder builder = new ArborescenceOutWriteResult.Builder();
            ...
            ZhuLiuOut zhuliu = computationResult.algorithm();
            SpanningTree spanningTree = computationResult.result().get();
            ArborescenceConfig config = computationResult.config();
