...
import org.neo4j.gds.arborescence.ArborescenceInAlgorithmFactory;
import org.neo4j.gds.arborescence.ZhuLiuIn;
import java.util.stream.Stream;

import static org.neo4j.gds.executor.ExecutionMode.MUTATE_RELATIONSHIP;

@GdsCallable(name = "gds.arborescence.in.write",
             description = ArborescenceInWriteProc.DESCRIPTION,
             executionMode = MUTATE_RELATIONSHIP)
public class ArborescenceInWriteSpec implements AlgorithmSpec<ZhuLiuIn, SpanningTree, ArborescenceConfig, Stream<ArborescenceInWriteResult>, ArborescenceInAlgorithmFactory<ArborescenceConfig>> {

    private final TransactionContext txContext;

    public ArborescenceInWriteSpec(TransactionContext txContext) {
        this.txContext = txContext;
    }

    public ArborescenceInWriteSpec() {
        this.txContext = null;
    }
    ...
    @Override
    public String name() {
        return "ArborescenceInWrite";
    }

    @Override
    public ArborescenceInAlgorithmFactory<ArborescenceConfig> algorithmFactory(ExecutionContext executionContext) {
        return new ArborescenceInAlgorithmFactory<>();
    }
    ...
    public ComputationResultConsumer<ZhuLiuIn, SpanningTree, ArborescenceConfig, Stream<ArborescenceInWriteResult>> computationResultConsumer() {

        return (computationResult, executionContext) -> {
            ArborescenceInWriteResult.Builder builder = new ArborescenceInWriteResult.Builder();
            ...
            ZhuLiuIn zhuliu = computationResult.algorithm();
