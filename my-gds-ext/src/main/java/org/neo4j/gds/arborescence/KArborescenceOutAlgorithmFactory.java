...
public class KArborescenceOutAlgorithmFactory<CONFIG extends KArborescenceConfig> extends GraphAlgorithmFactory<ZhuLiuOutK, CONFIG> {

    @Override
    public ZhuLiuOutK build(Graph graphOrGraphStore, CONFIG configuration, ProgressTracker progressTracker) {
        if (graphOrGraphStore.schema().isUndirected()) {
            throw new IllegalArgumentException(
                "The K Arborescence algorithm works only with directed graphs. Please orient the edges properly");
        }
        return new ZhuLiuOutK(
            graphOrGraphStore,
    ...
    public MemoryEstimation memoryEstimation(CONFIG config) {
        return MemoryEstimations.builder(ZhuLiuInK.class)
    ...
    public Task progressTask(Graph graph, CONFIG config) {
        return Tasks.task(
                taskName(),
                Tasks.leaf("KArborescenceOut", graph.nodeCount()),
