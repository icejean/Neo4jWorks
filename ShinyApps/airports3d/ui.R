...
fluidPage(
  ...
  tags$head(
    tags$script(HTML("
                ...
                // 触发了shiny:inputchanged事件，从浏览器端更新状态提示，因为服务器端要执行完了才返回。
                // 在触发服务器reactive()与observe之前先更新Browser端状态，因为耗时较长。
                // 清除选择结点更新状态在Shiny.addCustomMessageHandler()执行完了再设置，不在这里更新。
                $(document).on('shiny:inputchanged', function(event) {
                    var appstatus = document.getElementById('appstatus');
                    if (event.name === 'airport') {
                        appstatus.value = '正在查询网络数据...';
                    }
                    if (event.name === 'hops') {
                        appstatus.value = '正在查询网络数据...';
                    }
                    if (event.name === 'ifShotestPath') {
                        appstatus.value = '正在查询网络数据...';
                    }
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # 是否只返回最短路径
      checkboxInput("ifShotestPath","只看最短路径", value = TRUE),
      ...
      tags$h6(" "),
      textInput("appstatus","APP状态", value ="正在查询网络数据..."),
    ...
    mainPanel(
      ...
      # Load scripts for Vasturiano 3D Force-Directed Graph
      ...
      # 注册一个javascript消息处理函数，接收服务器发来的json图数据，
      # 更新图用HTML(), 否则javascript lambda表达式的=>箭头会被替换成&转义符，导致javascript执行出错。
